                    ...
                    <div className="flex flex-col flex-nowrap gap-3">
                        <div className="gap-3 space-y-3 md:flex md:space-y-0">
                            <div className="item md:w-96 lg:min-w-[30rem]">
                                <input type="text" placeholder="Deployment ID" {...register("deploymentID", { required: true })} className="input input-bordered w-full" />
                            </div>

                            <div className="item md:w-36">
                              
                                <input type="text" placeholder="Amount" {...register("amount", {})} className="input input-bordered w-full" disabled={watchActionType === "reallocate" || watchActionType === "allocate" ? false : true } />
                            ...
                            <div className="gap-3 md:flex">
                                <div className="item min-w-[27rem]">
                                    <input placeholder="Allocation ID" {...register("allocationID", { shouldUnregister: true, })} className="input input-bordered w-full" disabled={watchActionType === "reallocate" || watchActionType === "unallocate" ? false:true} />
                                </div>
                            </div>
                            ...
                            <div className="gap-3 md:flex">
                                <div className="item min-w-[27rem]">
                                    <input placeholder="POI" {...register("poi", { shouldUnregister: true, })} className="input input-bordered w-full" disabled={watchActionType === "reallocate" || watchActionType === "unallocate" ? false:true}/>
                                </div>
                                <div className="form-control w-28 justify-center">
                                    <label className="label cursor-pointer">
                                        <span className="label-text">Force</span>
                                    <input type="checkbox" placeholder="Force" {...register("force", { shouldUnregister: true, })} className="toggle toggle-accent" disabled={watchActionType === "reallocate" || watchActionType === "unallocate" ? false : true} />
                                    </label>
                                </div>
                            </div>
