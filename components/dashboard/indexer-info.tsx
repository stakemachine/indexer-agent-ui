		...
		<div className="space-y-4">
						...
						<div className="space-y-4">
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									>
										{indexerRegistration?.registered
											? "Registered"
											: "Not Registered"}
									</Badge>
								</div>
								<div className="text-sm font-mono">
									{indexerRegistration?.address || "N/A"}
								</div>
							</div>
							<div className="space-y-2">
								<div className="flex items-center justify-between">
			...
			<div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<StatCard
					title="Self stake"
					value={`${(
						(data.indexer.stakedTokens - data.indexer.lockedTokens) /
							1000000000000000000
					).toFixed(0)}`}
				/>
				<StatCard
					title="Allocated"
					value={`${(
						data.indexer.allocatedTokens / 1000000000000000000
					).toFixed(0)}`}
				/>
				<StatCard
					title="Unallocated"
					value={`${(data.indexer.availableStake / 1000000000000000000).toFixed(
						0,
					)}`}
				/>
				<StatCard
					...
					value={`${(
						data.indexer.delegatedCapacity / 1000000000000000000
					).toFixed(0)}`}
				/>
			</div>
		</div>
